...
def calculate_stochastic_rsi(data, n=14, k=3):
    ...
    data["SRSI_K"] = data["stoch_rsi"].rolling(k).mean() * 100
    data["SRSI_D"] = data["SRSI_K"].rolling(k).mean()
...
def backtest_strategy(stock, start_date, logfile):
    ...
    for i in range(len(data)):
        # Buy signal
        if data["SRSI_K"][i] > 20 and data["SRSI_D"][i] > 20 and position == 0:
            position = 1
            buy_price = data["Adj Close"][i]
            #print(f"Buying {stock} at {buy_price}")

        # Sell signal
        elif data["SRSI_K"][i] < 80 and data["SRSI_D"][i] < 80 and position == 1:
            position = 0
