...
def backtest_strategy(stock, start_date):
    ...
    for ma in range ( 5, 40 ):
        ...
        for i in range(len(data)):
            # Buy signal
            if data["Adj Close"][i] > data["EMA_{}".format(ma)][i] and data["Adj Close"][i - 1] < data["EMA_{}".format(ma)][i - 1] and position == 0:
                position = 1
            ...
            # Sell signal
            elif data["Adj Close"][i] < data["EMA_{}".format(ma)][i] and data["Adj Close"][i - 1]  > data["EMA_{}".format(ma)][i - 1] and position == 1:
                position = 0
