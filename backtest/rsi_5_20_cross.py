...
def backtest_strategy(stock, start_date, logfile):
    ...
    for i in range(len(data)):
        ...
        # Buy signal
        if data["RSI_5"][i] > data["RSI_20"][i] and data["RSI_5"][i - 1] < data["RSI_20"][i - 1] and position == 0:
            position = 1
            buy_price = data["Adj Close"][i]
            #print(f"Buying {stock} at {buy_price}")

        # Sell signal
        elif data["RSI_5"][i] < data["RSI_20"][i] and data["RSI_5"][i - 1] > data["RSI_20"][i - 1]  and position == 1:
            position = 0
