...
def backtest_strategy(stock, start_date, logfile):
    ...
    for i in range(len(data)):
        # Buy signal
        if (   data["TEMA_30"][i] > data["SMA_20"][i] ) and ( data["TEMA_30"][i - 1] <= data["SMA_20"][i - 1] ) and (position == 0):
            position = 1
            buy_price = data["Adj Close"][i]
            #print(f"Buying {stock} at {buy_price}")

        # Sell signal
        elif ( data["TEMA_30"][i] < data["SMA_20"][i] ) and ( data["TEMA_30"][i - 1] >= data["SMA_20"][i - 1] ) and position == 1:
            position = 0
