...
def backtest_strategy(stock, start_date, logfile):
    ...
    for i in range(len(data)):
        ...
        # Buy signal
        if ( position ==0 ) and ( (data['MFI_14'][i] > 20) & ( data['MFI_14'][i - 1] <= 20 ) & ( data['STO_K'][i] > 20 ) & ( data['STO_K'][i - 1] <= 20)):
            position = 1
            buy_price = data["Adj Close"][i]
            #print(f"Buying {stock} at {buy_price}")

        # Sell signal
        elif ( position == 1 ) and ( ( data['MFI_14'][i] < 80 ) & ( data['MFI_14'][i - 1] >= 80 ) & ( data['STO_K'][i] < 80 ) & ( data['STO_K'][i - 1] >= 80) ):
            position = 0
