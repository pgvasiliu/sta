...
def backtest_strategy(stock, start_date, logfile):
    ...
    for i in range(len(data)):
        ...
        # Buy signal
        if ( position == 0 ) and  (  ( data["RSI_14"][i]  >= 30 ) & ( data["RSI_14"][i - 1]  < 30 ) & ( data['TEMA_9'][i]  <= data['BB_middle'][i] ) & ( data['TEMA_9'][i]  > data['TEMA_9'][i - 1] ) & ( data["Volume"][i]  > 0) ):
            position = 1
            buy_price = data["Adj Close"][i]
            #print(f"Buying {stock} at {buy_price}")

        # Sell signal
        elif ( position == 1 ) and (  ( data["RSI_14"][i] >=70 ) & ( data["RSI_14"][i - 1] < 70 ) & ( data['TEMA_9'][i] > data['BB_middle'][i] ) & ( data['TEMA_9'][i] < data['TEMA_9'][i - 1] ) & ( data["Volume"][i]  > 0) ):
            position = 0
