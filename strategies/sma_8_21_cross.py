...
def backtest_strategy(stock, start_date):
    ...
    for i in range(len(data)):
        # Buy signal
        if data["SMA_8"][i] > data["SMA_21"][i] and data["SMA_8"][i - 1] < data["SMA_21"][i - 1] and position == 0:
            position = 1
        ...
        # Sell signal
        elif data["SMA_8"][i] < data["SMA_21"][i] and data["SMA_8"][i - 1]  > data["SMA_21"][i - 1] and position == 1:
            position = 0
...
if data["SMA_8"][-1] > data["SMA_21"][-1] and data["SMA_8"][-2] < data["SMA_21"][-2]:
    ...
    plot ( "sma_8_21_cross.py", ticker, FILE, interval )
