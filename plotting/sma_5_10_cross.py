...
for symbol in args.ticker:
    ...
    data["Signal"] = 0.0
    data['SMA_5_10_Signal'] = np.select(
        [ ( data['SMA_5'].shift(1) <  data['SMA_10'].shift(1) ) & ( data['SMA_5'] >  data['SMA_10'] ) ,
          ( data['SMA_5'].shift(1) >  data['SMA_10'].shift(1) ) & ( data['SMA_5'] <  data['SMA_10'] ) ],
    [2, -2])
    ...
    plt.plot ( data["SMA_5"], alpha = 0.6, linewidth = 2, color='orange',  label = 'SMA_5',  )
    plt.plot ( data["SMA_10"], alpha = 0.6, linewidth = 3, color='#FF006E', label = 'SMA_10' )

    plt.plot ( data.loc[data["SMA_5_10_Signal"] ==  2.0].index, data["SMA_5"][data["SMA_5_10_Signal"] ==  2.0], "^", markersize=10, color="g", label = 'BUY SIGNAL')
    plt.plot ( data.loc[data["SMA_5_10_Signal"] == -2.0].index, data["SMA_5"][data["SMA_5_10_Signal"] == -2.0], "v", markersize=10, color="r", label = 'SELL SIGNAL')
