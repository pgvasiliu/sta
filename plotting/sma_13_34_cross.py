...
for symbol in args.ticker:
    ...
    data["SMA_34"] = data["Adj Close"].rolling(window=34).mean()

    data = data.dropna()

    # Buy/sell signals for  SMA crosses
    data["SMA_13_34_Signal"] = 0.0
    data['SMA_13_34_Signal'] = np.select(
        [ ( data['SMA_13'].shift(1) <  data['SMA_34'].shift(1) ) & ( data['SMA_13'] >  data['SMA_34'] ) ,
          ( data['SMA_13'].shift(1) >  data['SMA_34'].shift(1) ) & ( data['SMA_13'] <  data['SMA_34'] ) ],
    [2, -2])
    ...
    plt.plot ( data["SMA_34"], alpha = 0.6, linewidth = 3, color='#FF006E', label = 'SMA_34' )

    plt.plot ( data.loc[data["SMA_13_34_Signal"] ==  2.0].index, data["SMA_13"][data["SMA_13_34_Signal"] ==  2.0], "^", markersize=10, color="g", label = 'BUY SIGNAL')
    plt.plot ( data.loc[data["SMA_13_34_Signal"] == -2.0].index, data["SMA_13"][data["SMA_13_34_Signal"] == -2.0], "v", markersize=10, color="r", label = 'SELL SIGNAL')
