...
for symbol in args.ticker:
    ...
    data['Signal'] = np.select(
        [ ( data['SMA_20'].shift(1) <  data['TEMA_30'].shift(1) ) & ( data['SMA_20'] >  data['TEMA_30'] ) ,
          ( data['SMA_20'].shift(1) >  data['TEMA_30'].shift(1) ) & ( data['SMA_20'] <  data['TEMA_30'] ) ],
    [-2, 2])
    ...
    plt.plot ( data["TEMA_30"], alpha = 0.6, linewidth = 2, color='blue', label = 'TEMA_30' )

    plt.plot ( data.loc[data["Signal"] ==  2.0].index, data["SMA_20"][data["Signal"] ==  2.0], "^", markersize=10, color="g", label = 'BUY SIGNAL')
    plt.plot ( data.loc[data["Signal"] == -2.0].index, data["SMA_20"][data["Signal"] == -2.0], "v", markersize=10, color="r", label = 'SELL SIGNAL')
