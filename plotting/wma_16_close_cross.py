...
for symbol in args.ticker:
    ...
    data["WMA_16_close_Signal"] = 0.0
    data['WMA_16_close_Signal'] = np.select(
        [ ( data['WMA_16'].shift(1) <  data['Adj Close'].shift(1) ) & ( data['WMA_16'] >  data['Adj Close'] ) ,
          ( data['WMA_16'].shift(1) >  data['Adj Close'].shift(1) ) & ( data['WMA_16'] <  data['Adj Close'] ) ],
    [2, -2])
    ...
    plt.plot ( data["WMA_16"], alpha = 0.6, linewidth = 2, color='orange',  label = 'WMA_16',  )

    plt.plot ( data.loc[data["WMA_16_close_Signal"] ==  -2.0].index, data["WMA_16"][data["WMA_16_close_Signal"] ==  -2.0], "^", markersize=10, color="g", label = 'BUY SIGNAL')
    plt.plot ( data.loc[data["WMA_16_close_Signal"] == 2.0].index, data["WMA_16"][data["WMA_16_close_Signal"] == 2.0], "v", markersize=10, color="r", label = 'SELL SIGNAL')
